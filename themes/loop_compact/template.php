...
function loop_compact_preprocess_node(&$variables) {
  ...
  // Fetch user metadata.
  $variables['author_name'] = _loop_compact_fetch_full_name($author);
  ...
  $variables['author_image'] = _loop_compact_fetch_author_image($author);
  ...
  if ($variables['type'] == 'post') {
    if (!empty($variables['field_file_upload'])) {
      $variables['files'] = _loop_compact_fetch_files('node', $variables['node']);
    }
  }
 ...
 */
function loop_compact_preprocess_block(&$variables) {
  // Skip login formular.
 ...
 */
function loop_compact_preprocess_panels_pane(&$variables) {
  if (arg(0) == 'editor') {
  ...
  if ($variables['pane']->subtype == 'user_messages-panel_pane_1' || $variables['pane']->subtype == 'user_messages-panel_pane_5') {
    ...
    $variables['message_count'] = _loop_compact_fetch_user_new_notifications();
 ...
 */
function loop_compact_preprocess_search_api_page_results(&$variables) {
 ...
 */
function loop_compact_menu_local_task($variables) {
  $secondary = '';
 ...
 */
function loop_compact_menu_local_tasks($variables) {
  $output = '';
 ...
 * inside.
 */
function loop_compact_fieldset($variables) {
  $element = $variables['element'];
 ...
 */
function loop_compact_preprocess_user_profile(&$variables) {
  $account = $variables['elements']['#account'];

  $variables['full_name'] = _loop_compact_fetch_full_name($account);
 ...
 */
function loop_compact_panels_default_style_render_region($vars) {
  $output = '';
 ...
 */
function loop_compact_form_user_login_alter(&$form) {
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function loop_compact_form_views_form_loop_user_subscriptions_panel_pane_1_alter(&$form, &$form_state, $form_id) {
  // Add form class.
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function loop_compact_form_views_form_loop_user_taxonomy_subscriptions_panel_pane_1_alter(&$form, &$form_state, $form_id) {
  // Add form class.
  $form['#attributes']['class'][] = 'vbo-views-form';
 ...
 */
function loop_compact_form_user_register_form_alter(&$form) {
 ...
 */
function loop_compact_form_views_exposed_form_alter(&$form) {
  if (arg(1) == 'dashboard') {
 ...
 * field.
 */
function loop_compact_form_search_api_page_search_form_default_alter(&$form) {
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function loop_compact_form_comment_form_alter(&$form) {
  $variables['user_obj'] = user_load($GLOBALS['user']->uid);
  if (!empty($variables['user_obj'])) {
    $variables['user_name'] = _loop_compact_fetch_full_name($variables['user_obj']);
    ...
    $variables['place'] = $wrapper->field_location_place->value();
    $variables['author_image'] = _loop_compact_fetch_author_image($variables['user_obj']);
  }
 ...
 */
function loop_compact_form_user_profile_form_alter(&$form) {
 ...
 */
function loop_compact_form_notifications_account_manage_subscriptions_form_alter(&$form) {
  // Hide filtering.
 ...
 */
function loop_compact_theme($existing, $type, $theme, $path) {
  return array(
 ...
 */
function loop_compact_preprocess_comment(&$variables) {
  // Make the content author object available.
  $variables['comment']->account = user_load($variables['comment']->uid);

  $variables['comment_author_name'] = _loop_compact_fetch_full_name($variables['comment']->account);
  $variables['comment_author_image'] = _loop_compact_fetch_author_image($variables['comment']->account);

  $variables['comment_body'] = _loop_compact_fetch_comment_body($variables['comment']);
  ...
  if ($variables['node']->type == 'post') {
    if (!empty($variables['field_file_upload_comment'])) {
      $variables['files'] = _loop_compact_fetch_files('comment', $variables['comment']);
    }
  }
 ...
 */
function loop_compact_preprocess_loop_post_subscription_list(&$vars) {
 ...
 */
function loop_compact_textarea($variables) {
  $element = $variables['element'];
 ...
 * overrides).
 */
function loop_compact_preprocess_views_view(&$vars) {
  // We run the new message count in this function since the view updates with
  // ajax.
  if ($vars['view']->name == 'user_messages') {
    $new_message_count = _loop_compact_fetch_user_new_notifications();
 ...
 *   not logged in.
 */
function _loop_compact_print_notification_tab() {
 ...
 */
function _loop_compact_fetch_full_name ($user) {
  $name = '';
 ...
 */
function _loop_compact_fetch_user_new_notifications() {
 ...
 */
function _loop_compact_fetch_author_image($author) {
  $author_image = '';
 ...
 *   Comment safe value.
 */
function _loop_compact_fetch_comment_body($comment) {
  // Load entity wrapper.
 ...
 */
function _loop_compact_fetch_files($type, $entity) {
  $files = FALSE;
